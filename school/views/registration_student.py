...
from school.serializers.registration_student import ListRegistrationStudentSerializer


class ListRegistrationStudentView(generics.ListAPIView):

    def get_queryset(self):
        queryset = RegistrationModel.objects.filter(student_id=self.kwargs["pk"])
        return queryset
